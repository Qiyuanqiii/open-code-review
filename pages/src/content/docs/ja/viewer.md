...
各セッションについて：ID（UUID）、VCS、該当する場合のブランチ名、要求されたレビュー入力、
レビューモード、モデル、ファイル数、所要時間、開始タイムスタンプ。

### `/r/{repo}/{sessionID}`——セッション詳細

詳細ページが最も有用です。以下を表示します：

1. **ヘッダー**——VCS、要求された入力、モデル、該当する場合のブランチ、合計 token、実行時間。
   不変 SVN セッションでは、認証情報を含まない manifest の解決済み数値 endpoint と peg revision
   も表示します。repository target URL と認証情報は表示しません。
