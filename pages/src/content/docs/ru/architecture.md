...
появления JSON в терминале. Оно поможет сформировать представление о системе,
необходимое для диагностики поведения, настройки флагов и уверенного чтения
исходного кода.
...
который включает четыре файла: `agent.go` (основной цикл и диспетчеризация),
`compression.go` (сжатие памяти), `preview.go` (фильтр файлов) и `util.go`
(вспомогательные функции). Важны две точки входа: `Agent.Run` (начало
конвейера) и `Agent.dispatchSubtasks` (распределение по отдельным файлам).

## Провайдер diff

В `internal/diff/git.go` определена структура `Provider`, закрытое поле `mode`
...
…либо пустое значение, если файл оставлен. `deleted` **не** возвращается
функцией `whyExcluded`: оно вычисляется позже в `Preview()`, когда diff
оставленного файла сообщает `IsDeleted`. Проверки выполняются в таком порядке:
...
Шумные каталоги (`vendor/`, `node_modules/`, `target/` и т. д.) фильтруются
раньше, на уровне провайдера diff, с помощью списка `providerDirIgnoreDirs` в
`internal/diff/git.go`: diff для таких каталогов разбираются, а затем
удаляются функцией `filterDiffs`, не доходя до фильтра отдельных файлов.
...
текст через заполнитель `{{plan_tools}}` (формируется функцией
...
после чего заполнители шаблона подставляются отдельно для каждого файла:
...
Подстановка заполнителей реализована в
...
> **Особенность синтаксиса заполнителей.** Все перечисленные выше заполнители
...
- **Обнаружение эндпоинта не имеет резервного варианта.** Если конфигурация,
