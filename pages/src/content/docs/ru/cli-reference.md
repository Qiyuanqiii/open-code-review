...
| `--from <ref>` | — | — | Исходная ссылка Git, от которой начинается diff (например, `main`). |
| `--to <ref>` | — | — | Целевая ссылка Git, на которой заканчивается diff (например, `feature-branch`). Если задана, OCR вычисляет `merge-base(from, to)..to`. |
| `--commit <sha>` | `-c` | — | Один коммит Git для ревью (сравнивается с родительским). |
...
> диапазона или коммита и несовместим с `--preview`. Subversion поддерживает
> только режим рабочей области.
...
изменений исходного кода с привязкой к строкам. Для SVN не поддерживаются
`--from`, `--to` и `--commit`.
