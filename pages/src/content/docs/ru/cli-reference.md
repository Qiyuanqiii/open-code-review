...
Используйте этот режим после добавления нужных изменений в индекс, в том числе
отдельных фрагментов файла. OCR сохраняет весь индекс как Git tree, один раз
разрешает `HEAD` и проверяет разницу между ними. В репозитории без коммитов базой
служит пустое дерево. Последующие правки, изменения индекса или перемещения
ветки не меняют уже зафиксированный вход ревью. Если diff индекса пуст, LLM
не вызывается.

Файлы `.gitattributes` репозитория читаются из зафиксированного дерева. Глобальные
атрибуты, `.git/info/attributes` и настройки Git остаются внешними входами.
Минимальная версия Git по-прежнему 2.41.

Diff, встроенное чтение файлов, поиск файлов и поиск по содержимому используют
одно дерево, включая неизменённые файлы, нужные для контекста. Например, если
версия с ошибкой находится в индексе, а исправление — только в рабочем дереве,
ревью читает версию из индекса. Изменения вне индекса и неотслеживаемые файлы
не включаются. Правила репозитория по умолчанию из `.opencodereview/` тоже
читаются из снимка. Размер каждого файла правил, включая `rule.json`, ограничен
512 КиБ. Документы правил должны быть обычными файлами снимка с путями относительно
репозитория; отсутствующие файлы, абсолютные пути и символические ссылки вызывают
ошибку без возврата к чтению с диска. Явно заданные файлы `--rule`, `--exclude`, глобальные настройки
и другие внешние данные остаются пользовательскими входами. Гарантия снимка
для встроенных инструментов не распространяется на пользовательские MCP-серверы
и другие внешние инструменты.

Файлы в индексе уже отслеживаются, поэтому `.gitignore` рабочего дерева не
исключает их повторно. Стандартные исключения OCR для каталогов и секретных
файлов, список поддерживаемых файлов и исключения правил ревью продолжают
действовать. Через `--preview` можно проверить выбранные файлы, причины
исключения и идентификатор снимка до расходования токенов. Каждый новый запуск
создаёт новый снимок.

Этот режим использует `ocr.run-manifest/v2`: `input.mode` равен `staged`,
`input.snapshot_tree` обозначает дерево индекса, а `input.resolved_base` —
зафиксированный коммит `HEAD` (отсутствует до первого коммита). ID дерева не
является ID коммита; `resolved_head` и `exact_range` не заполняются. Другие режимы
ревью сохраняют v1. Потребители, поддерживающие только v1, включая первую версию
проверки review gate, не могут использовать результаты staged v2 как совместимые
данные для такой проверки. JSON-предпросмотр содержит те же поля идентификации
в объекте `input`.

Команда не выполняет stash, reset или checkout файлов и не меняет пользовательский
индекс и refs. Она может записывать объекты tree без ссылок; впоследствии их
удаляет обычная сборка мусора Git. Связанные рабочие деревья поддерживаются.
В этой версии неразрешённые конфликты слияния, записи intent-to-add (`git add -N`),
split index и sparse index отклоняются с ошибкой. `--staged` несовместим с
`--from`, `--to`, `--commit` и `--resume`; сессии ревью индекса нельзя возобновить.

Изменённые записи submodule/gitlink также отклоняются до чтения содержимого
файлов: это относится к добавлению, обновлению, удалению и преобразованию между
обычным файлом и gitlink. Неизменённые подмодули не мешают ревью обычных файлов;
их пути gitlink исключаются из `file_find` и отклоняются в `file_read`.
`code_search` не выполняет поиск внутри подмодулей, даже если рекурсия включена в настройках Git.
