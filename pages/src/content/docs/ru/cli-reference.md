...
| `--no-filter` | — | `false` | Пропускает вызов LLM постобработки `REVIEW_FILTER_TASK` для каждой подзадачи; фильтры результатов продолжают действовать. Подзадача проверяет один файл или набор связанных файлов. |
| `--min-severity <level>` | — | — | Выводит находки не ниже указанной важности: `critical`, `high`, `medium` или `low`. Находки с неизвестной важностью или категорией всегда сохраняются. |
| `--exclude-categories <list>` | — | — | Исключает категории, перечисленные через запятую: `bug`, `security`, `performance`, `maintainability`, `test`, `style`, `documentation`, `other`. Находки с неизвестной важностью или категорией всегда сохраняются. |
...
### Фильтрация выводимых находок

```bash
ocr review --min-severity medium --exclude-categories style,maintainability,test
```

Порог важности включает указанный уровень. Регистр не учитывается, пробелы по краям удаляются; неподдерживаемые значения вызывают ошибку до начала ревью. При использовании обоих флагов находка исключается, если её важность ниже порога или категория входит в список исключений. Если хотя бы одно из этих полей отсутствует или не распознано, находка сохраняется независимо от другого фильтра.

Фильтры применяются при выводе результатов, включая частичные результаты и результаты возобновлённых сессий. Поддерживаются текст, JSON (включая число комментариев) и SARIF. Фильтрация влияет только на отчёт: объём работы LLM и покрытие файлов остаются прежними, а исходные находки сохраняются в сессии. При возобновлении можно задать другие фильтры. `--no-filter` пропускает постобработку LLM; фильтры отчёта применяются независимо.
...
Отсутствующая или неизвестная важность проходит фильтр `--severity`, а отсутствующая или неизвестная категория — фильтр `--category`. Поля с распознанными значениями фильтруются обычным образом. Например, `--severity high` исключает находку с важностью `low`, даже если её категория неизвестна. Регистр и пробелы по краям не учитываются.
...
в persisting. Если манифест сессии after содержит переименование файла, перед
сопоставлением старый путь заменяется новым.
