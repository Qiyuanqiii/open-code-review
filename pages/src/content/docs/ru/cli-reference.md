...
| Код | Значение |
|---|---|
| `0` | Ревью завершено или пропущено (возможно, без комментариев или с некритическими предупреждениями). Частичное ревью, вызванное только настроенными ограничениями бюджета, также завершается с кодом `0`. |
| `1` | Критическая ошибка или операционная ошибка отдельной группы, например тайм-аут, ошибка провайдера или конфигурации. Частичные результаты всё равно публикуются до возврата ошибки. |

Предупреждения выводятся в процессе работы; в режиме JSON они добавляются в
массив `warnings`. Затронутые файлы и классификации ошибок доступны в
