...
В рабочей копии Subversion 1.7+ OCR автоматически определяет SVN и объединяет
Git-совместимый вывод `svn diff --git` с неверсионируемыми файлами из
`svn status --xml`. Изменения только свойств игнорируются, поскольку в них нет
изменений исходного кода с привязкой к строкам.
