...
Основная команда. Определяет diff Git или рабочей копии Subversion, объединяет
семантически связанные файлы в группы, запускает по субагенту на группу,
...
Если флаги не переданы, OCR работает в **режиме рабочей области** и проверяет
все локальные изменения рабочей копии Git или Subversion в текущем каталоге.
...
Для Git OCR формирует изменения рабочего дерева с помощью двух команд:
...
В рабочей копии Subversion OCR сначала проверяет возможности командой
`svn --version --quiet`. Для используемого Git-совместимого diff требуется
клиент версии 1.7 или новее. В режиме рабочей копии OCR объединяет вывод
`svn diff --git` с неверсионируемыми файлами из `svn status --xml`. Изменения
только свойств игнорируются, поскольку в них нет изменений исходного кода с
привязкой к строкам.

Граничные случаи рабочего пространства SVN имеют явную семантику:

- Копия показывается как добавление целого файла с путём copy-from относительно
  корня репозитория и номером ревизии. Перемещение — это такое добавление плюс
  отдельное удаление. Замена также показывается как удаление и добавление.
- Изменения двоичного содержимого и файлы с двоичным MIME-типом помечаются как
  binary. Изменения только свойств executable, EOL, MIME и других свойств
  игнорируются, так как у них нет построчного изменения исходного кода. Если
  меняется и содержимое, оно сохраняется в diff, а текст свойств удаляется.
- Рабочие копии с разными BASE-ревизиями поддерживаются: каждый узел сравнивается
  со своей BASE. Состояния switched, sparse, obstructed, incomplete и conflicted
  отклоняются до загрузки diff.
- Определения `svn:externals` и извлечённые external-узлы отклоняются. Запускайте
  OCR отдельно в корне каждой внешней рабочей копии, чтобы файловые инструменты
  не пересекали границу выбранного репозитория.
- Если консоль Windows теряет символы Unicode, источником истины служат XML-пути.
  Пробелы и `@` в именах сохраняются; команды SVN обращаются только к корню
  рабочей копии после `--`, поэтому имя не разбирается как peg-синтаксис.
- Проверка использует фиксированное число рекурсивных команд SVN, а не процесс
  на каждый файл. Команды выполняются последовательно, ограниченно и с отменой.
...
Для SVN оба значения являются ревизиями, и OCR сравнивает точные
зафиксированные конечные точки:

```bash
ocr review --from 120 --to 128
```

Поддерживаются числовые ревизии, `HEAD` и `{DATE}`. Подвижные значения и даты
разрешаются в общие для репозитория числа до загрузки diff, вызова файловых
...
Для SVN `--commit 128` проверяет точное изменение `127:128` в пределах URL
рабочей копии. Ревизия 0 является пустым входом, поскольку у неё нет
предшественника. Содержимое новых файлов и инструменты поиска читают ревизию
128, а не текущее рабочее дерево.
