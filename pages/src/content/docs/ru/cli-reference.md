...
| `--staged` | — | `false` | Проверяет только изменения в индексе относительно зафиксированного `HEAD`; встроенные инструменты читают код из неизменного снимка индекса. |
...
> `--commit`, либо `--staged`, либо ничего (режим рабочей области).
...
Этот режим включает изменения вне индекса и неотслеживаемые файлы. Чтобы
проверить только подготовленные к следующему коммиту изменения, используйте `--staged`.

#### Режим снимка индекса
...
Этот режим проверяет изменения, подготовленные в индексе для следующего коммита,
включая отдельные фрагменты файлов. OCR сохраняет весь индекс как Git tree и
сравнивает его с зафиксированным `HEAD`. До первого коммита базой служит пустое
дерево. Снимок входных данных остаётся неизменным на протяжении ревью. При пустом
diff индекса вызов LLM пропускается. Требуется Git 2.41 или новее; связанные
рабочие деревья поддерживаются.

Diff, встроенное чтение файлов, поиск файлов и поиск по содержимому используют
одно дерево, включая неизменённые файлы контекста. Файлы `.gitattributes` и
правила репозитория по умолчанию из `.opencodereview/` также читаются из этого
дерева. Размер каждого файла правил, включая `rule.json`, ограничен 512 КиБ.
Указанные в правилах документы должны быть обычными файлами снимка с путями
относительно репозитория. Отсутствующие файлы, абсолютные пути и символические
ссылки вызывают ошибку.

Явно заданные файлы `--rule`, `--exclude`, глобальная конфигурация, глобальные
атрибуты Git, `.git/info/attributes` и настройки Git остаются внешними входами.
Пользовательские MCP-серверы и другие внешние инструменты могут читать текущее
состояние рабочего дерева или внешних систем.

`.gitignore` не исключает пути из индекса. Действуют стандартные исключения OCR
для каталогов и секретных файлов, список поддерживаемых файлов и исключения
правил ревью. `--preview` показывает выбранные файлы, исключения и идентификатор
снимка без вызова модели. Каждый запуск создаёт собственный снимок.

Ревью индекса использует `ocr.run-manifest/v2`: `input.mode` равен `staged`,
`input.snapshot_tree` содержит ID объекта Git tree, а `input.resolved_base` —
зафиксированный коммит `HEAD` (поле отсутствует до первого коммита).
`resolved_head` и `exact_range` опускаются. JSON-предпросмотр содержит те же поля
идентификации в `input`. Для обработки результатов нужна поддержка v2;
потребители, поддерживающие только v1, должны сообщать о неподдерживаемом формате.
Другие режимы ревью сохраняют v1.

Пользовательский индекс, рабочее дерево и refs остаются неизменными. Временные
копии индекса удаляются, а объекты tree без ссылок подчиняются обычному жизненному
циклу сборки мусора Git.

`--staged` несовместим с `--from`, `--to`, `--commit` и `--resume`.
Сессии ревью индекса нельзя возобновить. Неразрешённые
конфликты слияния, записи intent-to-add (`git add -N`), split index и sparse index
вызывают ошибку. Изменённые записи submodule/gitlink также вызывают ошибку до
чтения содержимого файлов: это относится к добавлению, обновлению, удалению и
преобразованию между обычным файлом и gitlink.

Репозитории с неизменёнными подмодулями поддерживают ревью обычных файлов.
`file_find` пропускает пути gitlink, `file_read` отклоняет их, а `code_search`
пропускает содержимое подмодулей независимо от настроек рекурсии Git.
Символические ссылки читаются как сохранённые blob самой ссылки, без перехода
к её цели.
...
- ревью рабочей области и индекса нельзя возобновить;
