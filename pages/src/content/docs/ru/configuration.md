...
Некоторые LLM-шлюзы используют нестандартные HTTP-коды 4xx для временных
...
Допускаются только HTTP-коды 4xx. Для `408`, `409` и `429` SDK уже выполняет
повторные попытки, поэтому при чтении файла конфигурации OCR игнорирует эти
избыточные коды. Если задать их через `ocr config set`, OCR также выводит
