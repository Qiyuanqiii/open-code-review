...
## Hooks репозитория Subversion и автоматизация слияний

Серверная автоматизация SVN использует точные old/new цели, а не семантику Git
merge-base. В репозитории есть повторно используемый скрипт
...
Пять аргументов — каталог конфигурации OCR, исходная operative-ревизия,
исходная цель, целевая operative-ревизия и целевая цель. Hook `post-commit` или
сервис слияний должен получить их из payload события и поставить выполнение
скрипта в очередь за пределами блокировки репозитория. С абсолютными URL каталог
`/srv/ocr-config` может быть обычным каталогом; checkout не требуется.

Скрипт выводит JSON и не принимает учётные данные в аргументах. До запуска
подготовьте для сервисной учётной записи кэш аутентификации SVN и доверенный
сертификат сервера. Все команды SVN неинтерактивны; отсутствие аутентификации
или доверия сертификату немедленно приводит к ошибке с рекомендацией. Целевые
URL не могут содержать userinfo, query string или fragment и не попадают в
manifest OCR, журналы session, вывод delegate или telemetry.
