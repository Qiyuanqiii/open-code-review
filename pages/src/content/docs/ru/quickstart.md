...
- **Git ≥ 2.41** для ревью Git или **Subversion ≥ 1.7** для ревью рабочей области и ревизий SVN
- **Node.js ≥ 18**
...
Перейдите в Git-репозиторий или рабочую копию SVN и выполните:

```bash
cd path/to/your-repo

# Рабочая копия Git — все локальные изменения, включая новые файлы
ocr review
...
# История ревизий SVN — перед ревью конечные точки фиксируются как числа
ocr review --commit 128
ocr review --from 120 --to 128
```

В рабочей копии SVN команда `ocr review` без аргументов включает версионируемые
и неверсионируемые локальные изменения. `--commit REV` проверяет `REV-1:REV`,
а `--from REV --to REV` сравнивает две ревизии репозитория. `HEAD` и ревизии по
дате разрешаются в числовые ревизии до загрузки diff.
